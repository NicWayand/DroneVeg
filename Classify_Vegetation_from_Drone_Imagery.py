...
# Load stuff in (Chunk using dask, to avoid OOM)
ds_dsm  = xr.open_rasterio(DSM_Tif, chunks={'x':1000, 'y':1000}).sel(band=1).drop('band')
ds_dsm  = ds_dsm.where(ds_dsm>0).astype(np.float32).load()
...
window_meters = 11 # m # should be odd, so center of filter is one pixel. Size depends on vegetatioin in domain.
# Should be larger than max clusters of forest (you want the filter size to always see the ground around the
# Vegetation.
pixel_meters = 0.05 # m/pixel (Resoution of input DSM)
